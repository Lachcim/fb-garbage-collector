...
def remove_failed_posts(self):
    ...
    for post in posts:
        if post["admin"]:
            logging.info("post by {0} allowed due to adminship".format(post["author"]))
            continue
            
        if now - post["time"] >= 86400:
            logging.info("post by {0} grandfathered in".format(post["author"]))
            continue
            
        if now - post["time"] >= 3600 and post["likeCount"] >= 50:
            logging.info("post by {0} passed".format(post["author"]))
            continue
            
        if now - post["time"] < 3600:
            logging.info("post by {0} isn't old enough ({1:.2f} hours)".format(post["author"], (now - post["time"]) / 3600))
            continue
        ...
        logging.info("post by {0} marked for deletion, {1} likes reached in {2:.2f} hours".format(post["author"], post["likeCount"], (now - post["time"]) / 3600))
